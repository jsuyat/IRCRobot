...
class IRC_Server:
    ...
    def connect(self):
        ...
        #Custom - added by our group
        self.print_help(self.irc_channel)
        ######
        self.listen()
...
# This function takes a channel, which must start with a #.
    ...
    #This is a custom method added by our group
    def print_help(self, channel):
        self.send_message_to_channel( ("Default Distance: 20 cm, Default Speed: 20 cm/s, Default Rotation: 45°"), channel )
        self.send_message_to_channel( ("Available Commands:"), channel )
        for command in self.lexer.COMMANDS:
            self.send_message_to_channel( (command), channel )
        self.send_message_to_channel( ("Type !help to repeat these commands"), channel )
    #####
...
# For now, we only have 2: root admin, and anyone.
    ...
    # All public commands go here
        self.lexer.parse(channel, user, command)
    
# Here begins the main programs flow:
test = IRC_Server("irc.rizon.net", 6667, "R42_Ness", "#irobot")
